...
void AnalysisUser::execute() {
  ...
  for(int i=0; i<candidates; i++) {
    pass_cut[i] = false;
     if(TQ_channel[i]!=1 && TQ_channel[i]!=2) continue;
     num_eve[2]++;
    ...
    && trk3_pT[i]>1000 && trk4_pT[i]>1000 && Muon1_charge[i]*Muon2_charge[i]<0
    && fabs(Muon1_eta[i])<2.5 && fabs(Muon2_eta[i])<2.5 && fabs(trk3_eta[i])<2.5 && fabs(trk4_eta[i])<2.5
    && fabs(trk5_eta[i])<2.5 && fabs(trk6_eta[i])<2.5 
    )
    {
      pass_cut[i] = true;
    }
  ...
  mu1.SetPtEtaPhiM(Muon1_pT[itq], Muon1_eta[itq], Muon1_phi[itq], 105.658);
  mu2.SetPtEtaPhiM(Muon2_pT[itq], Muon2_eta[itq], Muon2_phi[itq], 105.658);
  ...
  if (Ld_type[itq] != 3){ //Ld
    Ld_window = ((ld_m/GeV> 0.9 && ld_m/GeV< 1.3) || (ld_rf_m/GeV> 0.9 && ld_rf_m/GeV<1.3));
  }
  else { //Ks
    Ld_window = ((ld_m/GeV> 0.25 && ld_m/GeV< 0.75) || (ld_rf_m/GeV> 0.25 && ld_rf_m/GeV<0.75));
  }
  Jpsi_window = ((jpsi_m/GeV>2.5 && jpsi_m/GeV<3.6) || (jpsi_rf_m/GeV>2.5 && jpsi_rf_m/GeV<3.6));
